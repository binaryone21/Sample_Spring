...
public class ExcelTool {
	...
	public static final void writeUsingTemplate(List<Map<String, Object>> dataList, String newExcelPath, String templatePath) throws Exception {
		Workbook templateWorkbook = getWorkbook(templatePath);
		boolean isXSSF = isXSSF(templateWorkbook);
		Workbook newWorkbook = createWorkbook(isXSSF);
		...
		for (int templateSheetIdx = 0; templateSheetIdx < templateSheetCount; templateSheetIdx++) {
			...
			// 매핑 로우 찾기
			int templateMappingRowIdx = getMappingRowIdx(templateSheet);
			if (templateSheetIdx == -1) {
				logger.warn("매핑로우가 존재하지 않습니다. [" + templateSheet.getSheetName() + "]");
				copySheet(templateSheet, newWorkbook);
				continue;
			}

			int templateDataRowIdx = templateMappingRowIdx + 1;	// 템플릿 시트의 데이터 로우 인덱스
			...
			int mappingCellCount = templateMappingRow.getLastCellNum() - 1;	// 매핑 셀 개수(첫번째 셀 제외)
			int dataCellCount = templateDataRow.getLastCellNum() - 1;	// 데이타 셀 개수(첫번째 셀 제외)
			// 데이터 셀 개수가 매핑 셀 개수보다 적을 수는 없다.
			if(dataCellCount < mappingCellCount) {
			...
			CellAttrVO[] templateDataCellAttrArr = new CellAttrVO[dataCellCount];	// 템플릿 데이터 셀 속성 배열
			// 매핑명 세팅
			for(int columnIdx=0; columnIdx<mappingCellCount; columnIdx++) {
				mappingNameArr[columnIdx] = StringUtils.trim(String.valueOf(templateMappingRow.getCell(columnIdx + 1)));
			}
			...
			for(int dataCellIdx=0; dataCellIdx<dataCellCount; dataCellIdx++) {
				Cell templateCell = templateDataRow.getCell(dataCellIdx + 1);
				CellAttrVO cellAttrVO = null;
				if(templateCell != null) {
					CellStyle cs = newWorkbook.createCellStyle();
					cs.cloneStyleFrom(templateCell.getCellStyle());
					cs.setWrapText(true);

					// 개행문자 처리
					cs.setWrapText(true);
					cellAttrVO = new CellAttrVO(cs, templateCell.getCellType(), getValue(templateCell));
				}
				templateDataCellAttrArr[dataCellIdx] = cellAttrVO;
			}
			...
			if(isXSSF) {
				...
				int availableDataRowCount = 65536 - headerRowCount;
				...
				for(int newSheetIdx=0; newSheetIdx<newSheetCount; newSheetIdx++) {
					...
					int dataBeginIdx = newSheetIdx * availableDataRowCount;
					int dataEndIdx = (newSheetIdx + 1) * availableDataRowCount - 1;
					if(dataEndIdx > (dataCount - 1)) {
